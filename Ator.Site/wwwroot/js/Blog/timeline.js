...
$(function () {
    var _html = '';
    $.get("/Home/GetTimeLineData", function (res) {
        ...
        for (var i = 0; i < datas.length; i++) {
            ...
            var monthDatas = datas[i].months;
            console.log(JSON.stringify(monthDatas));
            for (var k = 0; k < monthDatas.length; k++) {
                _month = monthDatas[k];
                console.log(_month);
                var _data = _month.data;
                console.log(JSON.stringify(_data));
                _html += '<div class="timeline-month">';
                _html += '<h3 class=" animated fadeInLeft"><a class="monthToggle" href="javascript:;">' + _month.month + '月</a><i class="fa fa-caret-down fa-fw"></i></h3>';
                _html += '<ul>';
